...
TEST(elfTests, isValidDecrypt) {
  ...
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("", "./tests/files/elf/isValidDecryptDecrypted_1.elf"), "^Error: Empty original path argument! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Accepted empty argument");          // Empty
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt(" ", "./tests/files/elf/isValidDecryptDecrypted_1.elf"), "^Error: Empty original path argument! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Accepted whitespace argument");    // Single space
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("  ", "./tests/files/elf/isValidDecryptDecrypted_1.elf"), "^Error: Empty original path argument! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Accepted whitespace argument");   // Double space
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("\t", "./tests/files/elf/isValidDecryptDecrypted_1.elf"), "^Error: Empty original path argument! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Accepted whitespace argument");   // Single tab
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("\t\t", "./tests/files/elf/isValidDecryptDecrypted_1.elf"), "^Error: Empty original path argument! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Accepted whitespace argument"); // Double tab
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt(nullptr, "./tests/files/elf/isValidDecryptDecrypted_1.elf"), "^basic_string::_M_construct null not valid$", "Accepted nullptr argument");                                           // nullptr

  // Original, Non-existant file
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/doesNotExist.ext", "./tests/files/elf/isValidDecryptDecrypted_1.elf"), "^Error: Input original path does not exist or is not a file! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Opened non-existant file");

  // Original, Open non-file object
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/notAFile.ext", "./tests/files/elf/isValidDecryptDecrypted_1.elf"), "^Error: Input original path does not exist or is not a file! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Opened non-file object as file");

  // Original, Open file without permission to access
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/noPermission.ext", "./tests/files/elf/isValidDecryptDecrypted_1.elf"), "^Error: Cannot open original file: \\./tests/files/elf/noPermission\\.ext at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Could \"open\" file without permissions");

  // Original, File is not a SELF
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/brokenSelfMagic.self", "./tests/files/elf/isValidDecryptDecrypted_1.elf"), "^Error: Input original path is not a SELF! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Passed a file that was not a SELF");

  // Decrypted, Empty input arguments
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/isValidDecryptEncrypted_1.self", ""), "^Error: Empty decrypted path argument! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Accepted empty argument");          // Empty
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/isValidDecryptEncrypted_1.self", " "), "^Error: Empty decrypted path argument! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Accepted whitespace argument");    // Single space
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/isValidDecryptEncrypted_1.self", "  "), "^Error: Empty decrypted path argument! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Accepted whitespace argument");   // Double space
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/isValidDecryptEncrypted_1.self", "\t"), "^Error: Empty decrypted path argument! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Accepted whitespace argument");   // Single tab
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/isValidDecryptEncrypted_1.self", "\t\t"), "^Error: Empty decrypted path argument! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Accepted whitespace argument"); // Double tab
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/isValidDecryptEncrypted_1.self", nullptr), "^basic_string::_M_construct null not valid$", "Accepted nullptr argument");                                            // nullptr

  // Decrypted, Non-existant file
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/isValidDecryptEncrypted_1.self", "./tests/files/elf/doesNotExist.ext"), "^Error: Input decrypted path does not exist or is not a file! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Opened non-existant file");

  // Decrypted, Open non-file object
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/isValidDecryptEncrypted_1.self", "./tests/files/elf/notAFile.ext"), "^Error: Input decrypted path does not exist or is not a file! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Opened non-file object as file");

  // Decrypted, Open file without permission to access
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/isValidDecryptEncrypted_1.self", "./tests/files/elf/noPermission.ext"), "^Error: Cannot open decrypted file: \\./tests/files/elf/noPermission\\.ext at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Could \"open\" file without permissions");

  // Decrypted, File is not an ELF
  EXPECT_EXCEPTION_REGEX(elf::is_valid_decrypt("./tests/files/elf/isValidDecryptEncrypted_1.self", "./tests/files/elf/brokenElfMagic.elf"), "^Error: Input decrypted path is not an ELF! at \"elf\\.cpp\":\\d*:\\(is_valid_decrypt\\)$", "Passed a file that was not a SELF");
...
TEST(elfTests, decrypt) {
  ...
  EXPECT_EXCEPTION_REGEX(elf::decrypt("./tests/files/elf/isValidSelf.self", ""), "^Error: Empty output path argument! at \"elf\\.cpp\":\\d*:\\(decrypt\\)$", "Accepted empty argument");          // Empty
  EXPECT_EXCEPTION_REGEX(elf::decrypt("./tests/files/elf/isValidSelf.self", " "), "^Error: Empty output path argument! at \"elf\\.cpp\":\\d*:\\(decrypt\\)$", "Accepted whitespace argument");    // Single space
  EXPECT_EXCEPTION_REGEX(elf::decrypt("./tests/files/elf/isValidSelf.self", "  "), "^Error: Empty output path argument! at \"elf\\.cpp\":\\d*:\\(decrypt\\)$", "Accepted whitespace argument");   // Double space
  EXPECT_EXCEPTION_REGEX(elf::decrypt("./tests/files/elf/isValidSelf.self", "\t"), "^Error: Empty output path argument! at \"elf\\.cpp\":\\d*:\\(decrypt\\)$", "Accepted whitespace argument");   // Single tab
  EXPECT_EXCEPTION_REGEX(elf::decrypt("./tests/files/elf/isValidSelf.self", "\t\t"), "^Error: Empty output path argument! at \"elf\\.cpp\":\\d*:\\(decrypt\\)$", "Accepted whitespace argument"); // Double tab
  EXPECT_EXCEPTION_REGEX(elf::decrypt("./tests/files/elf/isValidSelf.self", nullptr), "^basic_string::_M_construct null not valid$", "Accepted nullptr argument");                                // nullptr

  // Output, Open non-file object
  EXPECT_EXCEPTION_REGEX(elf::decrypt("./tests/files/elf/isValidSelf.self", "./tests/files/elf/notAFile.ext"), "^Error: Output path exists, but is not a file! at \"elf\\.cpp\":\\d*:\\(decrypt\\)$", "Opened non-file object as file");

  // Output, Open file without permission to access
  EXPECT_EXCEPTION_REGEX(elf::decrypt("./tests/files/elf/isValidSelf.self", "./tests/files/elf/noPermission.ext"), "^Error: Cannot open output file: \\./tests/files/elf/noPermission\\.ext at \"elf\\.cpp\":\\d*:\\(decrypt\\)$", "Could \"open\" file without permissions");

  // Success, already ELF (Run twice to see if it works when the file already exists)
  std::filesystem::path decypted_output("./tests/files/elf/decryptedOutput.elf");
  if (std::filesystem::exists(decypted_output)) {
    std::filesystem::remove_all(decypted_output);
  }
